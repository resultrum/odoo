...
class SaleOrder(models.Model):
    ...
    def _get_invoice_status(self):
        ...
        for order in confirmed_orders:
            line_invoice_status = [d[1] for d in line_invoice_status_all if
                                   d[0] == order.id]
            if order.state not in ('sale', 'done'):
                ...
                order.invoice_status = 'to invoice'
                    ...
                    invoice_status == 'invoiced' for invoice_status in
                    line_invoice_status):
                order.invoice_status = 'invoiced'
            elif line_invoice_status and all(
                    invoice_status in ('invoiced', 'upselling') for
    ...
    def _search_invoice_ids(self, operator, value):
        if operator == 'in' and value:
            ...
            order_ids = self._search([
                ('order_line.invoice_lines.move_id.move_type', 'in',
                 ('out_invoice', 'out_refund'))
            ])
            return [('id', 'not in', order_ids)]
        return ['&', ('order_line.invoice_lines.move_id.move_type', 'in',
                      ('out_invoice', 'out_refund')),
    ...
    currency_id = fields.Many2one(related='pricelist_id.currency_id',
                                  depends=["pricelist_id"], store=True,
                                  ondelete="restrict")
    ...
    invoice_ids = fields.Many2many("account.move", string='Invoices',
                                   compute="_get_invoiced", copy=False,
    ...
    amount_untaxed = fields.Monetary(string='Untaxed Amount', store=True,
                                     compute='_amount_all', tracking=5)
    ...
    signature = fields.Image('Signature',
                             ...
                             copy=False, attachment=True, max_width=1024,
                             max_height=1024)
    ...
    amount_undiscounted = fields.Float('Amount Before Discount',
    ...
    tag_ids = fields.Many2many('crm.tag', 'sale_order_tag_rel', 'order_id',
                               'tag_id', string='Tags')
    ...
    def _compute_tax_totals_json(self):
        ...
        for order in self:
            ...
            tax_totals = account_move._get_tax_totals(order.partner_id,
                                                      tax_lines_data,
                                                      order.amount_total,
    ...
    def _compute_amount_undiscounted(self):
        for order in self:
            total = 0.0
            for line in order.order_line:
                total += (line.price_subtotal * 100) / (
                            100 - line.discount) if line.discount != 100 else (
    ...
    def _compute_type_name(self):
        for record in self:
            ...
            'draft', 'sent', 'cancel') else _('Sales Order')
    ...
    def _compute_analytic_account_id(self):
        for order in self:
            if not order.analytic_account_id:
                default_analytic_account = order.env[
    ...
    def onchange_partner_shipping_id(self):
        ...
        self.fiscal_position_id = self.env[
            'account.fiscal.position'].with_company(
    ...
    def _onchange_commitment_date(self):
        ...
        if (
                self.commitment_date and self.expected_date and self.commitment_date < self.expected_date):
            return {
                'warning': {
    ...
    def create(self, vals):
        ...
        if any(f not in vals for f in
            ...
            vals['partner_invoice_id'] = vals.setdefault('partner_invoice_id',
                                                         addr['invoice'])
            vals['partner_shipping_id'] = vals.setdefault('partner_shipping_id',
                                                          addr['delivery'])
    ...
    def _compute_field_value(self, field):
        if field.name == 'invoice_status' and not self.env.context.get(
            ...
            filtered_self = self.filtered(lambda so: (
                                                                 so.user_id or so.partner_id.user_id) and so._origin.invoice_status != 'upselling')
    ...
    def _name_search(self, name, args=None, operator='ilike', limit=100,
        ...
        return super(SaleOrder, self)._name_search(name, args=args,
                                                   operator=operator,
                                                   limit=limit,
    ...
    def _create_upsell_activity(self):
        ...
        for order in self:
            ...
            order_ref = ref % (
            order._name, order.id or order._origin.id, order.name)
            customer_ref = ref % (order.partner_id._name, order.partner_id.id,
    ...
    def action_quotation_sent(self):
        if self.filtered(lambda so: so.state != 'draft'):
            raise UserError(
                _('Only draft orders can be marked as sent directly.'))
        for order in self:
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        for order in self:
            ...
            if not any(not line.display_type for line in invoiceable_lines):
                continue
            invoice_line_vals = []
            invoice_line_vals, invoice_item_sequence = order._prepare_invoice_lines(
                invoice_item_sequence, invoiceable_lines, invoice_line_vals)
        ...
        if len(invoice_vals_list) < len(self):
            ...
            for invoice in invoice_vals_list:
                ...
                for line in invoice['invoice_line_ids']:
                    line[2][
                        'sequence'] = SaleOrderLine._get_invoice_line_sequence(
        ...
        if final:
            moves.sudo().filtered(lambda
                                      m: m.amount_total < 0).action_switch_invoice_into_refund_credit_note()
        for move in moves:
            move.message_post_with_view('mail.message_origin_link',
                                        values={'self': move,
                                                'origin': move.line_ids.mapped(
    ...
    def _prepare_invoice_lines(self, invoice_item_sequence, invoiceable_lines,
                               invoice_line_vals):
    ...
    def _show_cancel_wizard(self):
        for order in self:
            if order.invoice_ids.filtered(
                    lambda inv: inv.state == 'draft') and not order._context.get(
    ...
    def _find_mail_template(self, force_confirmation_template=False):
        ...
        if force_confirmation_template or (
                self.state == 'sale' and not self.env.context.get('proforma',
                                                                  False)):
    ...
    def message_post(self, **kwargs):
        ...
        return super(SaleOrder, self.with_context(
            mail_post_autofollow=self.env.context.get('mail_post_autofollow',
                                                      True))).message_post(
            **kwargs)
